...
parser = argparse.ArgumentParser(prog="Directory Sorting",description="Simple and Neat PyTube CLI Tools",epilog=f"{now.strftime('%Y/%m/%d, %H:%M:%S')}")
parser.add_argument("directory",help="Directory To Be Sorted",type=dir_path)
parser.add_argument("--include-hidden",help="Include Hidden Files",action="store_false",required=False,dest="include_hidden")
parser.add_argument("-v","--version", action="version",version=f"%(prog)s {__author__}:{__version__}")
...
class DirList:
  def __init__(self, dir=os.getcwd(), ignore_hidden=True):
    ...
    if temp_dir == os.path.expanduser("~") and not ignore_hidden:
      print("To prevent ruining system, we forbid to sort home directory without ignore hidden")
      exit()
    else:
      self.dir = dir
    ...
    for file in self._directory_files:
      ...
      for folder, extension in self._extensions.items():
        if ext in extension and os.path.splitext(file)[0] != "dirlist":
          try:
            shutil.move(os.path.join(self.dir, file),os.path.join(os.getcwd(),folder))
          except Exception as e:
            new_name = f"{os.path.splitext(file)[0]}_{random_number}.{ext}"
            shutil.move(os.path.join(self.dir, file),os.path.join(os.getcwd(),folder,new_name))
          logger.info(f"Successfully Moved {file} to {folder} Folder")
          matched = True
      if not matched and os.path.splitext(file)[0] != "dirlist":
        try:
          shutil.move(os.path.join(self.dir, file),os.path.join(os.getcwd(),"Other"))
        except Exception as e:
          new_name = f"{os.path.splitext(file)[0]}_{random_number}.{ext}"
          shutil.move(os.path.join(self.dir, file),os.path.join(os.getcwd(),"Other",new_name))
        logger.info(f"Successfully Moved {file} to {folder} Folder")
  @property
  ...
  def _extensions(self):
    ...
    extensions = {
        "Documents": ["doc", "docx", "txt", "pdf", "ppt", "pptx", "csv", "xlsx","xlsm", "md"],
